    ...
    try:
        ...
        for line in result.stdout.strip().split('\n'):
            if line.strip() and '│' in line:
                ...
                if len(parts) >= 2:
                    skills.append({"name": parts[0], "category": parts[1] if len(parts) > 1 else "", "description": parts[2] if len(parts) > 2 else ""})
...
def get_sessions_data():
    ...
    sessions = []
    for file_path in sorted(glob.glob(str(sessions_dir / "*.json")), reverse=True)[:50]:
        try:
            ...
            for msg in data.get("messages", [])[:3]:
                if msg.get("role") == "user":
                    ...
                    break
            sessions.append({"id": Path(file_path).stem, "title": data.get("title", ""), "created": data.get("created_at", ""), "messages": len(data.get("messages", [])), "preview": preview})
        except: continue
...
def get_projects_data():
    ...
    if memory_file.exists():
        for line in memory_file.read_text().split('\n'):
            if '🎯' in line or '项目' in line: projects.append(line.strip())
    return {"projects": projects if projects else ["默认项目 1", "默认项目 2"], "count": len(projects)}
    ...
    if sessions_dir.exists():
        for file_path in glob.glob(str(sessions_dir / "*.json")):
            try:
                ...
                for msg in data.get("messages", []): total_tokens += len(msg.get("content", "")) // 4
            except: continue
        ...
        for file_path in glob.glob(str(sessions_dir / "*.json")):
            try:
                ...
                if created:
                    ...
                    hourly_usage[str(dt.hour)] += 1
                    daily_usage[dt.strftime('%Y-%m-%d')] += 1
            except: continue
    peak = max(hourly_usage.keys(), key=lambda k: hourly_usage[k]) if any(hourly_usage.values()) else "N/A"
    return {"hourly": hourly_usage, "daily": dict(sorted(daily_usage.items(), reverse=True)[:14]), "peak_hour": peak}
...
async def chat(message: str = Form(...), image: UploadFile = File(None), file: UploadFile = File(None)):
    ...
    response = call_hermes(full_message or "请分析", image_path)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page-header h2 { font-size: 24px; color: #00d9ff; display: flex; align-items: center; gap: 10px; }
        .chat-messages { flex: 1; overflow-y: auto; padding: 30px; display: flex; flex-direction: column; gap: 20px; }
        .message { display: flex; gap: 15px; max-width: 80%; animation: fadeIn 0.3s ease; }
        ...
        .message.user .message-content { background: linear-gradient(135deg, #1f3a5f 0%, #2a4a6f 100%); border-color: #3a5a8f; }
        ...
        .action-btn { width: 44px; height: 44px; border-radius: 50%; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 20px; transition: all 0.2s ease; }
        ...
        .preview-remove, .file-preview-remove { background: #ff4757; color: white; border: none; width: 24px; height: 24px; border-radius: 50%; cursor: pointer; font-size: 14px; }
        .file-preview-remove { width: 28px; height: 28px; }
        .typing-indicator { display: flex; gap: 5px; padding: 15px 20px; }
        .typing-dot { width: 8px; height: 8px; border-radius: 50%; background: #00d9ff; animation: typing 1.4s infinite; }
        ...
        .stat-value { font-size: 36px; font-weight: bold; background: linear-gradient(135deg, #00d9ff 0%, #00ff88 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        ...
        .refresh-btn { background: #2a2a4e; color: #00d9ff; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 14px; }
        .refresh-btn:hover { background: #3a3a5e; }
        .empty-state { text-align: center; padding: 60px 20px; color: #666; }
        .clear-btn { width: 100%; padding: 10px; background: #2a2a4e; color: #ff6b6b; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; margin-top: 10px; }
        .clear-btn:hover { background: #3a3a5e; }
...
<body>
    <div class="sidebar">
        ...
        <div style="padding:10px; border-top:1px solid #1f3a5f;">
            <button class="clear-btn" onclick="clearChatHistory()">清空对话</button>
        </div>
    </div>
    <div class="main">
        <div id="page-chat" class="page active">
            ...
            <div class="preview-container" id="previewContainer">
                ...
                <button class="preview-remove" onclick="removeImage()" style="float:right;">X</button>
            </div>
            <div class="file-preview-container" id="filePreviewContainer">
                <div style="display:flex; align-items:center; gap:10px;">
                    <span id="filePreviewIcon" style="font-size:24px;"></span>
                    <div>
                        <div id="filePreviewName" style="color:#e8e8e8;font-size:14px;"></div>
                        <div id="filePreviewSize" style="color:#666;font-size:12px;"></div>
                    </div>
                    <button class="file-preview-remove" onclick="removeFile()">X</button>
                </div>
            </div>
            <div class="input-container">
                <div class="input-wrapper">
                    ...
                    <div class="input-actions">
                        <button class="action-btn upload-btn" onclick="document.getElementById('imageInput').click()" title="上传图片">Img</button>
                        <button class="action-btn upload-btn" onclick="document.getElementById('fileInput').click()" title="上传文件">File</button>
                        <button class="action-btn send-btn" id="sendBtn" onclick="sendMessage()">Send</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function getFileIcon(name) {
            ...
            var icons = {pdf:'📕', doc:'📘', docx:'📘', txt:'📄', xls:'📗', xlsx:'📗', csv:'📗', zip:'📦', rar:'📦', jpg:'🖼️', jpeg:'🖼️', png:'🖼️', gif:'🖼️', mp3:'🎵', mp4:'🎬', py:'🐍', js:'📜', html:'🌐', json:'📋'};
            return icons[ext] || '📄';
        }
        ...
        function handleFileSelect(e) {
            ...
            if (file) {
                currentFile = file;
                document.getElementById('filePreviewIcon').innerHTML = getFileIcon(file.name);
...
AT_KEY, JSON.stringify(msgs));
        ...
        function showLoading() {
            ...
            div.id = 'loadingMsg';
            div.innerHTML = '<div class="message-avatar">AI</div><div class="message-content"><div class="typing-indicator"><div class="typing-dot"></div><div class="typing-dot"></div><div class="typing-dot"></div></div></div>';
            chatMessages.appendChild(div);
            chatMessages.scrollTop = chatMessages.scrollHeight;
        }
        ...
        function loadMemory() { fetch('/api/memory').then(function(r){return r.json();}).then(renderList); }
        function loadSkills() { fetch('/api/skills').then(function(r){return r.json();}).then(renderList); }
        function loadSessions() { fetch('/api/sessions').then(function(r){return r.json();}).then(renderList); }
        function loadCron() { fetch('/api/cron').then(function(r){return r.json();}).then(renderList); }
        function loadProjects() { fetch('/api/projects').then(function(r){return r.json();}).then(renderList); }
        function loadCosts() { fetch('/api/costs').then(function(r){return r.json();}).then(renderStats); }
        function loadPatterns() { fetch('/api/patterns').then(function(r){return r.json();}).then(renderStats); }
        ...
        function renderList(data) {
            ...
            if (data.skills) {
                for (var i = 0; i < data.skills.length; i++) {
                    var s = data.skills[i];
                    html += '<li><strong>' + s.name + '</strong> <span style="color:#00d9ff;">' + s.category + '</span><br><small>' + (s.description||'') + '</small></li>';
                }
            } else if (data.sessions) {
                for (var i = 0; i < data.sessions.length; i++) {
                    var s = data.sessions[i];
                    html += '<li><strong>' + (s.title||'未命名') + '</strong><br><small>' + (s.created||'') + ' | ' + s.messages + '条消息</small></li>';
                }
            } else if (data.raw) {
                html = '<div class="card"><pre style="background:#0f0f1a;padding:15px;border-radius:8px;white-space:pre-wrap;">' + data.raw + '</pre></div>';
            ...
            html += '</ul></div>';
            var page = event.target.closest('.page').id.replace('page-','');
